...
**REST CLIENT:** в Visual Studio Code слева выбрать вкладку extensions, в поиск ввести Rest, установить, нажать на кнопку reload, когда установится. Создать файл в формате .http. Теперь туда можно ввести put, post, get, delete - он создаст шаблон. Над кодом будет небольшая надпись "Send request". После нажатия на которую справа откроется результат выполнения запроса. 200 - всё хорошо (или код, который вы ввели к Pl/SQL),если номер другой - то надо искать ошибку (это не очень просто сделать). Для удобства можно создавать процедуры с входными параметрами, чтобы убедиться в работоспособности части кода (но это не обязательно).
